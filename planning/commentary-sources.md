# Commentary Sources

## Sources Checked In This Pass

These sources were consulted to begin the commentary evidence table:

- Grant R. Osborne, `Revelation` in BECNT, via public Perlego preview.
- Grant R. Osborne, `Revelation Verse by Verse`, via public Perlego preview.
- G. K. Beale and David H. Campbell, `Revelation: A Shorter Commentary`, via accessible PDF excerpt.
- Brian J. Tabb, TGC Bible Commentary: Revelation, online.
- `An Exegetical Summary of Revelation 1-11`, online excerpt.
- NET notes and Mounce Reverse Interlinear public pages, as translation/reference controls rather than full commentary sources.

## Sources Still Needed

Highest priority:

- David E. Aune, `Revelation 1-5`, Word Biblical Commentary.
- G. K. Beale, `The Book of Revelation`, NIGTC.
- Craig R. Koester, `Revelation`, Anchor Yale Bible.
- Robert H. Mounce, `The Book of Revelation`, NICNT.
- H. B. Swete, `The Apocalypse of St John`.
- R. H. Charles, `A Critical and Exegetical Commentary on the Revelation of St John`.

Secondary:

- Ian Boxall, `The Revelation of Saint John`.
- Stephen S. Smalley, `The Revelation to John`.
- Craig S. Keener, `Revelation`, NIVAC.
- Robert L. Thomas, `Revelation 1-7`.
- Dennis Johnson, `Triumph of the Lamb`.
- Simon Kistemaker, `Exposition of the Book of Revelation`.

## Acquisition Notes

For each source, capture:

- exact edition,
- page numbers for Revelation 1:1-3,
- Greek text used if stated,
- treatment of `apokalypsis iesou christou`,
- treatment of `auto`,
- subject of `deixai`,
- referent of servants,
- `en tachei`,
- `esemanen`,
- `aposteilas`,
- angel referent,
- `ton logon tou theou`,
- `ten martyrian iesou christou`,
- `hosa/eiden` and `te`,
- any mention of ellipsis or structure.

## Citation Policy

- Use page-level citations for all claims in the manuscript.
- Use online previews only as temporary planning evidence unless the preview is stable enough for citation.
- Do not cite pirate or questionable scans in the manuscript.
- For copyrighted commentaries, paraphrase interpretive positions and quote only short, necessary phrases.
